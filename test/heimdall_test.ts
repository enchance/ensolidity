...
describe('Heimdall the gatekeeper', () => {
    ...
    // it('Test', async () => {
    //     // console.log('OWNER', await heimdallcore.OWNER())
    //     // console.log('PROJECT.OWNER', await heimdallcore.gkroles('PROJECT.OWNER'))
    //     // console.log('PROJECT.ADMIN', await heimdallcore.gkroles('PROJECT.ADMIN'))
    //     // console.log('CONTRACT', await heimdallcore.gkroles('CONTRACT'))
    // })
    ...
    it.only('Access', async () => {
        const purge_roles = async (user: SignerWithAddress, role: any = null, acct: any = owneruser) => {
            ...
            expect(await heimdallcore.connect(foouser).hasRole(HEIMDALL_OWNER, user.address)).is.false
            expect(await heimdallcore.connect(foouser).hasRole(HEIMDALL_CONTRACT, user.address)).is.false
        ...
        // Role admins
        expect(await heimdallcore.getRoleAdmin(HEIMDALL_OWNER)).equals(HEIMDALL_OWNER)
        expect(await heimdallcore.getRoleAdmin(HEIMDALL_CONTRACT)).equals(HEIMDALL_OWNER)
        ...
        await expect(heimdallcore.connect(owneruser).grantRole(HEIMDALL_OWNER, foouser.address)).is.revertedWith(NO_ACCESS)
        await expect(heimdallcore.connect(owneruser).grantRole(HEIMDALL_CONTRACT, foouser.address)).is.revertedWith(NO_ACCESS)
        ...
        {   // eslint-disable-line
            ...
            await heimdallcore.connect(hdlowner).revokeRole(HEIMDALL_CONTRACT, baruser.address)
